...
if [ -f "$ROOT_DIR/dist/gitty-linux-amd64" ]; then
  ...
  (cd "$ROOT_DIR" && GOOS=linux GOARCH=amd64 go build -ldflags='-s -w' -o "$PKG_DIR/usr/bin/gitty" ./cmd/gitty/)
fi
...
gitty (2.3.0) stable; urgency=medium
...
dpkg-deb --root-owner-group --build "$PKG_DIR" "$OUT_DIR/gitty_2.3.0_amd64.deb"

echo "[SUCCESS] Built: $OUT_DIR/gitty_2.3.0_amd64.deb"
